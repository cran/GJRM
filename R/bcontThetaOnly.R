bcontThetaOnly <- function(params, respvec, VC, ps, AT = FALSE){



# NEED TO MODIFY INPUT OF COPULA as I can not now allow for CDF, and that would be p1 and p2


p1 <- p2 <- pdf1 <- pdf2 <- c.copula.be2 <- c.copula.be1 <- c.copula2.be1be2 <- l.ln <- NA
...
##################
  ...
  l.par <- VC$weights*log(c.copula2.be1be2)
...
if( length(teta2) != 0) der2teta.derteta.stteta.st[teta.ind2] <- BITS2$der2teta.derteta.stteta.st 
  ...
  d2l.sigma21.sigma21  <- 1                  
  d2l.sigma22.sigma22  <- 1

  d2l.rho.rho          <- -VC$weights*( ((der2c.derrho.derrho*derteta.derteta.st^2 + der2h.derp1teta *der2teta.derteta.stteta.st)*c.copula2.be1be2 - der2h.derp1teta.st^2) /c.copula2.be1be2^2  ) 
