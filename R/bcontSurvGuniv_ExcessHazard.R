bcontSurvGuniv_ExcessHazard <- function(params, respvec, VC, ps, AT = FALSE){
  p1 <- p2 <- pdf1 <- pdf2 <- c.copula.be2 <- c.copula.be1 <- c.copula2.be1be2 <- NA

  n <- VC$n
  
  # Only c1 needed in RIGHT censoring case (as we are excluding quantity C.i, which is the only
  # place where d.rchrate is used)
  c1 <- VC$hrate
  ...
  dl.dbe1 <- -VC$weights*(   
      ...
      (1 - VC$cens)*c(p1^-1*dS1eta1)*dereta1derb1
