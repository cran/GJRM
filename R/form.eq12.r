form.eq12 <- function(formula.eq1, data, v1, margins, m1d, m2d, copSS = FALSE, inde = NULL, eq1.binsurv = FALSE){
  
    y1m <- f.eq1 <- f.eq1LI <- NULL
...
if(eq1.binsurv == FALSE){
    ...
    if(margins %in% c("tP","tNBI", "tNBII","tPIG") && min(y1.test, na.rm = TRUE) < 1) stop("The response of one or more margins must be greater than the truncation value.") 
    ...
    # the line above probit etc refers to survival margins...
    ...
    #no longer in use but it might be useful for DGP if in copula implementation
    #if(margins %in% c("P","ZTP","DGP","DGPII","DGP0")){ # no PIG, NBII, NBI as these are numerical (no need for y2m)
    # 
    #ly1 <- length(y1)
    #y1m <- list()
    #my1 <- max(y1)
    ...
    #
    #
    #if(max(y1) > 170 && margins %in% c("P","ZTP","DGP0") ) y1m <- mpfr( y1m, pmax(53, getPrec(y1))) 
    #
    #}
  ...
  list(f.eq1LI = f.eq1LI, formula.eq1 = formula.eq1, formula.eq1r = formula.eq1r, y1 = y1, y1.test = y1.test, y1m = NULL, f.eq1 = f.eq1)
