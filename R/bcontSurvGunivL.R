bcontSurvGunivL <- function(params, respvec, VC, ps, AT = FALSE){
...
l.par<- VC$weights*( VC$cens*( log(-dS1eta1) + log(Xd1P) ) + (1 - VC$cens)*log(1-p1) )
res<- -sum(l.par)
...
dl.dbe1<- -VC$weights*(   
   ...
   VC$cens*( c((dS1eta1*Xd1P)^-1)*(c(d2S1eta1*Xd1P)*dereta1derb1 + c(dS1eta1)*der2eta1dery1b1) ) - (1 - VC$cens)*c((1-p1)^-1*dS1eta1)*dereta1derb1
...
H <-  -(   
   ...
   crossprod(c(VC$weights*(1 - VC$cens)*((1-p1)^-2*dS1eta1^2+(1-p1)^-1*d2S1eta1))*dereta1derb1, dereta1derb1) -
   
   diag( colSums( t( t(c(VC$weights*(1 - VC$cens)*(1-p1)^-1*dS1eta1)*VC$X1)*der2.par1 ) ) ) 
