bconttwoParC <- function(params, respvec, VC, ps, AT = FALSE){
p1 <- p2 <- pdf1 <- pdf2 <- c.copula.be2 <- c.copula.be1 <- c.copula2.be1be2 <- NA
  ...
  if(is.null(VC$X3)){  
    ...
    nu.st      <- etan  <- params[(VC$X1.d2 + VC$X2.d2 + 3)] 
  ...
  if(!is.null(VC$X3)){  
    ...
    nu.st      <- etan  <- VC$X5%*%params[(VC$X1.d2 + VC$X2.d2 + VC$X3.d2 + VC$X4.d2 + 1):(VC$X1.d2 + VC$X2.d2 + VC$X3.d2 + VC$X4.d2 + VC$X5.d2)]
...
##################
  ...
  dHs1 <- distrHs(respvec$y1, eta1, sigma21, sigma21.st, 1, 1, margin2=VC$margins[1], naive = FALSE, min.dn = VC$min.dn, min.pr = VC$min.pr, max.pr = VC$max.pr, left.trunc = VC$left.trunc1)
...
if( is.null(VC$X3) ){
  ...
  H <- rbind( cbind(     be1.be1    , be1.be2        ,   be1.sigma21,            be1.sigma22,              be1.nu, be1.rho  ), 
              cbind( t(be1.be2)     , be2.be2        ,   be2.sigma21,            be2.sigma22,              be2.nu,  be2.rho  ), 
              cbind( t(be1.sigma21) , t(be2.sigma21) , sum(d2l.sigma21.sigma21), sum(d2l.sigma21.sigma22), sum(d2l.nu.sigma21), sum(d2l.rho.sigma21)),
              cbind( t(be1.sigma22) , t(be2.sigma22) , sum(d2l.sigma21.sigma22), sum(d2l.sigma22.sigma22), sum(d2l.nu.sigma22), sum(d2l.rho.sigma22)),
              cbind( t(be1.nu)      ,    t(be2.nu)   ,    sum(d2l.nu.sigma21),   sum(d2l.nu.sigma22),      sum(d2l.nu.nu),  sum(d2l.rho.nu) ),
              cbind( t(be1.rho)     ,   t(be2.rho)   ,   sum(d2l.rho.sigma21),   sum(d2l.rho.sigma22),     sum(d2l.rho.nu), sum(d2l.rho.rho) ) 
...
G   <- -c( colSums(       c(dl.dbe1)*VC$X1 ) ,
   ...
   nu.sigma21  <- crossprod(VC$X3*c(d2l.nu.sigma21),VC$X5) 
   nu.sigma22  <- crossprod(VC$X4*c(d2l.nu.sigma22),VC$X5)

   nu.nu       <- crossprod(VC$X5*c(d2l.nu.nu),VC$X5)
   rho.nu      <- crossprod(VC$X5*c(d2l.rho.nu),VC$X6)
  
      be1.nu     <- crossprod(VC$X1*c(d2l.be1.nu),VC$X5)
   be2.nu      <- crossprod(VC$X2*c(d2l.be2.nu),VC$X5)


    H <- rbind( cbind( be1.be1        ,   be1.be2      ,   be1.sigma21     , be1.sigma22    , be1.nu, be1.rho    ), 
                cbind( t(be1.be2)     ,   be2.be2      ,   be2.sigma21     , be2.sigma22    , be2.nu, be2.rho    ), 
                cbind( t(be1.sigma21) , t(be2.sigma21) ,   sigma21.sigma21 , sigma21.sigma22, nu.sigma21, rho.sigma21),
                cbind( t(be1.sigma22) , t(be2.sigma22) , t(sigma21.sigma22), sigma22.sigma22, nu.sigma22, rho.sigma22),
                cbind( t(be1.nu)      ,    t(be2.nu)   ,    t(nu.sigma21),   t(nu.sigma22),   nu.nu,      rho.nu ),
                cbind( t(be1.rho)     ,   t(be2.rho)   , t(rho.sigma21)    , t(rho.sigma22) , t(rho.nu), rho.rho    )  ) 
