bprobgHsCont2ROY <- function(params, respvec, VC, ps, AT = FALSE){
...
########  
 ...
 dHs  <- distrHs(respvec$y2, eta2, sigma1, sigma1.st, nu = 1, nu.st = 1, margin2=VC$margins[2], naive = FALSE, min.dn = VC$min.dn, min.pr = VC$min.pr, max.pr = VC$max.pr, left.trunc = VC$left.trunc1)
 ...
 dHs  <- distrHs(respvec$y3, eta3, sigma2, sigma2.st, nu = 1, nu.st = 1, margin2=VC$margins[3], naive = FALSE, min.dn = VC$min.dn, min.pr = VC$min.pr, max.pr = VC$max.pr, left.trunc = VC$left.trunc2)
...
if(VC$extra.regI == "sED") H <- regH(H, type = 2) 
 ...
 if(VC$margins[2] == "LN" || VC$margins[3] == "LN"){
    
   if(VC$margins[2] == "LN"){
 
     dHs  <- distrHsAT(exp(respvec$y2), eta2, sigma1, 1, margin2=VC$margins[2], min.dn = VC$min.dn, min.pr = VC$min.pr, max.pr = VC$max.pr)
  
     pdf2.M2 <- dHs$pdf2
     p2.M2   <- dHs$p2   
     h.M2    <- copgHs(p0[VC$inde0], p2.M2, eta1=NULL, eta2=NULL, teta1, teta.st1, Cop1, VC$dof1, min.dn = VC$min.dn, min.pr = VC$min.pr, max.pr = VC$max.pr)$c.copula.be2

   }
 
   if(VC$margins[3] == "LN"){

     dHs  <- distrHsAT(exp(respvec$y3), eta3, sigma2, 1, margin2=VC$margins[3], min.dn = VC$min.dn, min.pr = VC$min.pr, max.pr = VC$max.pr)
  
     pdf2.M3 <- dHs$pdf2
     p2.M3   <- dHs$p2  
     h.M3    <- copgHs(p0[VC$inde1], p2.M3, eta1=NULL, eta2=NULL, teta2, teta.st2, Cop2, VC$dof2, min.dn = VC$min.dn, min.pr = VC$min.pr, max.pr = VC$max.pr)$c.copula.be2 
 
   }
 
 l.parL[VC$inde0] <- log( h.M2 )     + log(pdf2.M2)
 l.parL[VC$inde1] <- log( 1 - h.M3 ) + log(pdf2.M3)  
      
 l.ln <- -sum(VC$weights*l.parL)   
         ...
         list(value = res, gradient = G, hessian = H, S.h = S.h, S.h1 = S.h1, S.h2 = S.h2, l = S.res, l.par = l.par, ps = ps, l.ln = l.ln,
              eta1 = eta1, eta2 = eta2, eta3 = eta3, sigma1.st = sigma1.st, sigma2.st = sigma2.st,
              ...
              p1 = p1, p0 = p0, pdf1 = pdf2.M2, pdf2 = pdf2.M3, c.copula2.be1be2 = c(c.copula2.be1be2.M2, c.copula2.be1be2.M3),         
