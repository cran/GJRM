bdiscrcont23 <- function(params, respvec, VC, ps, AT = FALSE){
...
##################
  ...
  if( length(teta1) != 0) dH11 <- copgHs(p1[teta.ind1], p2[teta.ind1], eta1=NULL, eta2=NULL, teta1, teta.st1, Cop1, VC$dof)
  if( length(teta2) != 0) dH12 <- copgHs(p1[teta.ind2], p2[teta.ind2], eta1=NULL, eta2=NULL, teta2, teta.st2, Cop2, VC$dof)
  h1  <- NA    
  if( length(teta1) != 0) h1[teta.ind1] <- dH11$c.copula.be2
  if( length(teta2) != 0) h1[teta.ind2] <- dH12$c.copula.be2      
    

  if( length(teta1) != 0) dH21 <- copgHs(mm(p1[teta.ind1]-pdf1[teta.ind1]), p2[teta.ind1], eta1=NULL, eta2=NULL, teta1, teta.st1, Cop1, VC$dof)
  if( length(teta2) != 0) dH22 <- copgHs(mm(p1[teta.ind2]-pdf1[teta.ind2]), p2[teta.ind2], eta1=NULL, eta2=NULL, teta2, teta.st2, Cop2, VC$dof)
  h2  <- NA    
  if( length(teta1) != 0) h2[teta.ind1] <- dH21$c.copula.be2
  if( length(teta2) != 0) h2[teta.ind2] <- dH22$c.copula.be2       
  
  diffh1.h2 <- h1 - h2 
  diffh1.h2 <- ifelse(diffh1.h2 < epsilon, epsilon, diffh1.h2)  
...
##################
 ...
 ####################
  ...
  if( length(teta1) != 0) BITS.H21 <- copgHsCont(mm(p1[teta.ind1]-pdf1[teta.ind1]), p2[teta.ind1], teta1, teta.st1, Cop1, Cont = TRUE, par2 = VC$dof, nu.st = log(VC$dof-2)) 
  if( length(teta2) != 0) BITS.H22 <- copgHsCont(mm(p1[teta.ind2]-pdf1[teta.ind2]), p2[teta.ind2], teta2, teta.st2, Cop2, Cont = TRUE, par2 = VC$dof, nu.st = log(VC$dof-2)) 
...
####################   
   ...
   dl.dsigma21.st <- VC$weights*( 1/diffh1.h2*(c.copula2.be1be2H1*derp1.dersigma21.st - c.copula2.be1be2H2*derp1m1.dersigma21.st)  )
   ...
   dl.dnu2.st     <- VC$weights*( derpdf2.dernu2.st/pdf2 + 1/diffh1.h2*(  (c.copula2.be2H1 -  c.copula2.be2H2)*derp2.dernu2.st )  )              
...
d2l.sigma21.sigma21 <- -VC$weights*( - (1/diffh1.h2*(c.copula2.be1be2H1*derp1.dersigma21.st - c.copula2.be1be2H2*derp1m1.dersigma21.st ))^2 + 1/diffh1.h2*( (der2h.derp1p1.H1*derp1.dersigma21.st^2  + c.copula2.be1be2H1*der2p1.dersigma21.st2) -  (der2h.derp1p1.H2*derp1m1.dersigma21.st^2  + c.copula2.be1be2H2*der2p1m1.dersigma21.st2)             ) )
...
d2l.be2.sigma21     <- -VC$weights*( - (c.copula2.be2H1 -  c.copula2.be2H2)*(derp2.dereta2/diffh1.h2^2)*(       c.copula2.be1be2H1*derp1.dersigma21.st  - c.copula2.be1be2H2*derp1m1.dersigma21.st   ) + diffh1.h2^-1*( der2h.derp1p2.H1*derp1.dersigma21.st   - der2h.derp1p2.H2*derp1m1.dersigma21.st )*derp2.dereta2 ) 
...
d2l.rho.sigma21     <- -VC$weights*( -(c.copula2.be2thH1 - c.copula2.be2thH2)/diffh1.h2^2*(c.copula2.be1be2H1*derp1.dersigma21.st  - c.copula2.be1be2H2*derp1m1.dersigma21.st   ) + diffh1.h2^-1*( der2h.derp1teta.st.H1*derp1.dersigma21.st - der2h.derp1teta.st.H2*derp1m1.dersigma21.st  )  )
...
d2l.be1.sigma21     <- -VC$weights*( - (1/diffh1.h2^2)*(c.copula2.be1be2H1*derp1.dersigma21.st - c.copula2.be1be2H2*derp1m1.dersigma21.st)*(c.copula2.be1be2H1*derp1.dereta1 - c.copula2.be1be2H2*derp1m1.dereta1 ) + diffh1.h2^-1*( der2h.derp1p1.H1*derp1.dereta1*derp1.dersigma21.st + c.copula2.be1be2H1*der2p1.dereta1dersigma21.st - (der2h.derp1p1.H2*derp1m1.dereta1*derp1m1.dersigma21.st + c.copula2.be1be2H2*der2p1m1.dereta1dersigma21.st)      )  )
...
d2l.sigma21.sigma22 <- -VC$weights*( - (c.copula2.be2H1 -  c.copula2.be2H2)*(derp2.dersigma22.st/diffh1.h2^2)*(       c.copula2.be1be2H1*derp1.dersigma21.st  - c.copula2.be1be2H2*derp1m1.dersigma21.st   ) + diffh1.h2^-1*( der2h.derp1p2.H1*derp1.dersigma21.st   - der2h.derp1p2.H2*derp1m1.dersigma21.st )*derp2.dersigma22.st )
...
d2l.nu2.nu2         <- -VC$weights*(  (der2pdf2.dernu2.st2*pdf2 - derpdf2.dernu2.st^2)/pdf2^2 - (c.copula2.be2H1 - c.copula2.be2H2)^2*derp2.dernu2.st^2/diffh1.h2^2+1/diffh1.h2*( der2h.derp2p2.H1*derp2.dernu2.st^2 + c.copula2.be2H1*der2p2.dernu2.st2-(der2h.derp2p2.H2*derp2.dernu2.st^2 + c.copula2.be2H2*der2p2.dernu2.st2) )  )
d2l.be2.nu2         <- -VC$weights*( (der2pdf2.dereta2dernu2.st*pdf2 - derpdf2.dereta2*derpdf2.dernu2.st)/pdf2^2 - (c.copula2.be2H1 - c.copula2.be2H2)*derp2.dernu2.st*(c.copula2.be2H1 - c.copula2.be2H2)*derp2.dereta2/diffh1.h2^2 + diffh1.h2^-1*( (der2h.derp2p2.H1-der2h.derp2p2.H2)*derp2.dereta2*derp2.dernu2.st + (c.copula2.be2H1-c.copula2.be2H2)*der2p2.dereta2dernu2.st)   )
d2l.be1.nu2         <- -VC$weights*( - (c.copula2.be2H1 -  c.copula2.be2H2)*(derp2.dernu2.st/diffh1.h2^2)*( c.copula2.be1be2H1*derp1.dereta1  - c.copula2.be1be2H2*derp1m1.dereta1   ) + diffh1.h2^-1*( der2h.derp1p2.H1*derp1.dereta1   - der2h.derp1p2.H2*derp1m1.dereta1 )*derp2.dernu2.st ) 
d2l.rho.nu2         <- -VC$weights*( -(c.copula2.be2thH1 - c.copula2.be2thH2)/diffh1.h2^2*((c.copula2.be2H1 -  c.copula2.be2H2)*derp2.dernu2.st) + diffh1.h2^-1*( der2h.derp2teta.st.H1 - der2h.derp2teta.st.H2  )*derp2.dernu2.st  )
d2l.sigma21.nu2     <- -VC$weights*( - (c.copula2.be2H1 -  c.copula2.be2H2)*(derp2.dernu2.st/diffh1.h2^2)*( c.copula2.be1be2H1*derp1.dersigma21.st  - c.copula2.be1be2H2*derp1m1.dersigma21.st   ) + diffh1.h2^-1*( der2h.derp1p2.H1*derp1.dersigma21.st   - der2h.derp1p2.H2*derp1m1.dersigma21.st )*derp2.dernu2.st ) 
d2l.sigma22.nu2     <- -VC$weights*( (der2pdf2.dersigma22.stdernu2.st*pdf2 - derpdf2.dersigma22.st*derpdf2.dernu2.st)/pdf2^2 - (c.copula2.be2H1 - c.copula2.be2H2)*derp2.dernu2.st*(c.copula2.be2H1 - c.copula2.be2H2)*derp2.dersigma22.st/diffh1.h2^2 + diffh1.h2^-1*( (der2h.derp2p2.H1-der2h.derp2p2.H2)*derp2.dersigma22.st*derp2.dernu2.st + (c.copula2.be2H1-c.copula2.be2H2)*der2p2.dersigma22.stdernu2.st)   )
