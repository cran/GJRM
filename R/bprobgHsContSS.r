...
if(!is.null(VC$X3)){  
 ...
 dHs  <- distrHs(respvec$y2, eta2, sigma2, sigma2.st, margin2=VC$margins[2], naive = FALSE, min.dn = VC$min.dn, min.pr = VC$min.pr, max.pr = VC$max.pr, left.trunc = VC$left.trunc2)
...
dl.dteta.stt[VC$inde]  <- dl.dteta.st	;dl.dteta.st  <- dl.dteta.stt
 ...
 l.ln <- -sum(VC$weights*l.par1)  
         ...
         list(value=res, gradient=G, hessian=H, S.h=S.h, S.h1=S.h1, S.h2=S.h2, l=S.res, l.par=l.par, ps = ps, etas = etas, l.ln=l.ln, 
