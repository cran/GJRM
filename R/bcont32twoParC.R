bcont32twoParC <- function(params, respvec, VC, ps, AT = FALSE){
...
##################
  ...
  dHs2 <- distrHs(respvec$y2, eta2, sigma22, sigma22.st,   1,      1, margin2=VC$margins[2], naive = FALSE, min.dn = VC$min.dn, min.pr = VC$min.pr, max.pr = VC$max.pr, left.trunc = VC$left.trunc2)
...
if( is.null(VC$X3) ){
  ...
  H <- rbind( cbind(     be1.be1    , be1.be2        ,   be1.sigma21,            be1.sigma22,              be1.nu1,               be1.nu, be1.rho  ), 
              cbind( t(be1.be2)     , be2.be2        ,   be2.sigma21,            be2.sigma22,              be2.nu1,                            be2.nu,  be2.rho  ), 
              cbind( t(be1.sigma21) , t(be2.sigma21) , sum(d2l.sigma21.sigma21), sum(d2l.sigma21.sigma22), sum(d2l.sigma21.nu1), sum(d2l.nu.sigma21), sum(d2l.rho.sigma21)),
              cbind( t(be1.sigma22) , t(be2.sigma22) , sum(d2l.sigma21.sigma22), sum(d2l.sigma22.sigma22), sum(d2l.sigma22.nu1), sum(d2l.nu.sigma22), sum(d2l.rho.sigma22)),
              cbind( t(be1.nu1)     , t(be2.nu1)     , sum(d2l.sigma21.nu1),     sum(d2l.sigma22.nu1),     sum(d2l.nu1.nu1),    sum(d2l.nu.nu1),     sum(d2l.rho.nu1)),
              cbind( t(be1.nu)      ,    t(be2.nu)   ,    sum(d2l.nu.sigma21),   sum(d2l.nu.sigma22),      sum(d2l.nu.nu1),      sum(d2l.nu.nu),  sum(d2l.rho.nu) ),
              cbind( t(be1.rho)     ,   t(be2.rho)   ,   sum(d2l.rho.sigma21),   sum(d2l.rho.sigma22),     sum(d2l.rho.nu1),     sum(d2l.rho.nu), sum(d2l.rho.rho) ) 
...
G   <- -c( colSums(       c(dl.dbe1)*VC$X1 ) ,
   ...
   nu.nu1      <- crossprod(VC$X5*c(d2l.nu.nu1),VC$X6) 
   ...
   be2.nu      <- crossprod(VC$X2*c(d2l.be2.nu),VC$X6)   
    ...
    H <- rbind( cbind( be1.be1        ,   be1.be2      ,   be1.sigma21     , be1.sigma22    , be1.nu1    ,  be1.nu, be1.rho    ), 
                cbind( t(be1.be2)     ,   be2.be2      ,   be2.sigma21     , be2.sigma22    , be2.nu1    ,  be2.nu, be2.rho    ), 
                cbind( t(be1.sigma21) , t(be2.sigma21) ,   sigma21.sigma21 , sigma21.sigma22, sigma21.nu1,  nu.sigma21, rho.sigma21),
                cbind( t(be1.sigma22) , t(be2.sigma22) , t(sigma21.sigma22), sigma22.sigma22, sigma22.nu1,  nu.sigma22, rho.sigma22),
                cbind( t(be1.nu1)     , t(be2.nu1)     , t(sigma21.nu1)    , t(sigma22.nu1) , nu1.nu1        , nu.nu1, rho.nu1    ),
                cbind( t(be1.nu)      ,    t(be2.nu)   ,    t(nu.sigma21),   t(nu.sigma22),   t(nu.nu1),     nu.nu,      rho.nu ),
                cbind( t(be1.rho)     ,   t(be2.rho)   , t(rho.sigma21)    , t(rho.sigma22) , t(rho.nu1) , t(rho.nu), rho.rho    )  ) 
...
if( VC$margins[1] == "LN" || VC$margins[2] == "LN"){
  ...
  if(VC$margins[2] == "LN") dHs1 <- distrHsAT(exp(respvec$y2), eta2, sigma22, 1, margin2 = VC$margins[2], min.dn = VC$min.dn, min.pr = VC$min.pr, max.pr = VC$max.pr)
  ...
  l.ln <- -sum( VC$weights*( log(pdf1) + log(pdf2) + log(c.copula2.be1be2) ) )
         ...
         list(value=res, gradient=G, hessian=H, S.h=S.h, S.h1=S.h1, S.h2=S.h2, l=S.res, l.par=l.par, ps = ps, 
              eta1=eta1, eta2=eta2, etad=etad, etas1 = etas1, etas2 = etas2, etan = etan, etan1 = etan1, l.ln = l.ln,
